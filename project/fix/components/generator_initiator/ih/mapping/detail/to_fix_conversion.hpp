#ifndef SIMULATOR_GENERATOR_INITIATOR_IH_MAPPING_DETAIL_TO_FIX_CONVERSION_HPP_
#define SIMULATOR_GENERATOR_INITIATOR_IH_MAPPING_DETAIL_TO_FIX_CONVERSION_HPP_
...
struct ToFixConverter<FixFieldType> {
  ...
  auto convert(ValueType value) -> int {
    ...
    fix::detail::to_fix_error::value_out_of_range<limits>(value);
  }
...
template <>
struct ToFixConverter<FIX::MDUpdateType> {
  static auto convert(core::enumerators::MarketDataUpdateType) -> int;
};

template <>
struct ToFixConverter<FIX::SubscriptionRequestType> {
  static auto convert(core::enumerators::MdSubscriptionRequestType) -> char;
};
...
auto convert_to_fix(const auto& value, TimestampPrecision precision)
  ...
  const auto utc_time_only =
      detail::ToFixConverter<FixFieldType>{}.convert(value);

  auto field = FixFieldType{};
  field.setString(FIX::UtcTimeOnlyConvertor::convert(
      utc_time_only, static_cast<int>(precision)));
  return field;
}

}  // namespace simulator::fix::generator_initiator

#endif  // SIMULATOR_GENERATOR_INITIATOR_IH_MAPPING_DETAIL_TO_FIX_CONVERSION_HPP_