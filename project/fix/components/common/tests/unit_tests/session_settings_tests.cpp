...
struct ToCoreFixSessionSettingsConverter : public FixSettings {
  auto make_fix_session_settings(const std::string_view content)
...
TEST_F(ToCoreFixSessionSettingsConverter, SetsDefaultHeadingForDefaultSetting) {
  const auto result = make_fix_session_settings(
      "[DEFAULT]\n"
      "ConnectionType=acceptor\n"
      "\n"
      "[SESSION]\n"
  ...
  EXPECT_THAT(result[0].heading, Eq("DEFAULT"));
}

TEST_F(ToCoreFixSessionSettingsConverter, DoesNotSetIdForDefaultSetting) {
  const auto result = make_fix_session_settings(
      "[DEFAULT]\n"
      "ConnectionType=acceptor\n"
      "\n"
  ...
  ASSERT_THAT(result, SizeIs(2));
  EXPECT_THAT(result[0].id, Eq(std::nullopt));
}

TEST_F(ToCoreFixSessionSettingsConverter, ParsesDefaultSettings) {
  const auto result = make_fix_session_settings(
      "[DEFAULT]\n"
      "ConnectionType=acceptor\n"
      "\n"
...
TEST_F(ToCoreFixSessionSettingsConverter, SetsSessionHeadingForSessionSetting) {
  ...
  ASSERT_THAT(result, SizeIs(2));
  EXPECT_THAT(result[1].heading, Eq("SESSION"));
}

TEST_F(ToCoreFixSessionSettingsConverter,
       SetsIdForSessionSettingFromBeginStringSenderCompIdTargetCompId) {
  const auto result = make_fix_session_settings(
      "[DEFAULT]\n"
      "ConnectionType=acceptor\n"
      "\n"
  ...
  ASSERT_THAT(result, SizeIs(2));
  EXPECT_THAT(result[1].id, Optional(Eq("FIXT.1.1:MktSimulator->QuodGateway")));
}

TEST_F(ToCoreFixSessionSettingsConverter, ParsesSessionSettings) {
  const auto result = make_fix_session_settings(
      "[DEFAULT]\n"
      "ConnectionType=acceptor\n"
      "\n"
...
TEST_F(ToCoreFixSessionSettingsConverter,
       FiltersDefaultSettingsFromSessionSettings) {
  const auto result = make_fix_session_settings(
      "[DEFAULT]\n"
      "ConnectionType=acceptor\n"
      "\n"
...
TEST_F(ToCoreFixSessionSettingsConverter,
       NotFilterSessionSettingWithDefaultKeyIfDefaultValueDiffersFromSession) {
  const auto result = make_fix_session_settings(
      "[DEFAULT]\n"
      "ConnectionType=acceptor\n"
      "\n"
