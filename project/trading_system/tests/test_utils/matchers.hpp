#ifndef SIMULATOR_TRADING_SYSTEM_TESTS_TEST_UTILS_MATCHERS_HPP_
#define SIMULATOR_TRADING_SYSTEM_TESTS_TEST_UTILS_MATCHERS_HPP_
...
MATCHER_P(ReturnsError, error_msg, "") {
  return ExplainMatchResult(IsFalse(), arg.has_value(), result_listener) &&
         ExplainMatchResult(StrEq(error_msg), arg.error(), result_listener);
}
...
inline auto operator<<(std::ostream& os,
                       const tl::expected<void, std::string>& expected)
    -> std::ostream& {
  if (expected.has_value()) {
    os << "expected{ value }";
  } else {
    os << "expected{ error: \"" << expected.error() << "\" }";
  }
  return os;
}
...
#endif  // SIMULATOR_TRADING_SYSTEM_TESTS_TEST_UTILS_MATCHERS_HPP_
