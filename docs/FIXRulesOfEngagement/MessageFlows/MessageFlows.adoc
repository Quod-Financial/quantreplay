...
This closing snapshot reflects the auction's outcome. The clearing price and volume are recorded as the Auction Clearing Price (269=Q). An opening auction additionally sets the Opening Price (269=4) and resets the session high and low prices (269=7 and 269=8) to it, tracking them from there on. An opening auction also resets the daily Trade Volume (269=B) to the auction's clearing volume, whereas closing and intraday auctions add their clearing volume to it (see <<message-flows-mkt-data-daily-reset>>). A closing auction always carries the last known Closing Price into the Previous Closing Price (269=e), then sets the new Closing Price (269=5) if it crossed. If an auction does not cross, no clearing price can be calculated: the Auction Clearing Price (269=Q), together with the Opening Price (269=4) or the (already carried) Closing Price (269=5) for that auction type, is cleared and omitted from updates until a price can next be determined. An opening auction that does not cross also clears the Trade Volume (269=B). The session high and low prices are left untouched.
...
The session-level L1 prices — Opening Price (269=4), Closing Price (269=5), Previous Closing Price (269=e) and the Trading Session Low (269=8), Trading Session High (269=7) and Mid Prices (269=H) — are normally reset by the opening and closing auctions, as described above. When a venue's phase schedule does not include the corresponding auction, the simulator instead resets these values to the day's trades so they still roll over once per day.
