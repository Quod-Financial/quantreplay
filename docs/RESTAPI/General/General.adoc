...
If column mapping is not specified, their default column order or names are used.
If column mapping is defined for at least one of the level columns (BidParty, BidPrice, BidQuantity, AskParty, AskPrice, AskQuantity), it must be specified for the rest of the BidPrice, BidQuantity, AskPrice, AskQuantity. BidParty and AskParty are optional columns.
...
In this case, if either ReceivedTimeStamp, MessageTimeStamp, Instrument or none of them are mapped, the unmapped columns are assumed to use the same values as available for the columnFrom property as the column name in the data source.
However, if any level column (BidParty, BidPrice, BidQuantity, AskParty, AskPrice, AskQuantity) is mapped, the other level columns (BidPrice, BidQuantity, AskPrice, AskQuantity) must also be mapped. BidParty and AskParty columns are optional.
