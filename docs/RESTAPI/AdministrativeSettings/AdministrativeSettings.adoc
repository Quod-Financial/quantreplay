...
| phases | Array | Scheduled phase changes
|=== 
...
*	Auction - intraday auction
*	TradeAtLast - trading at the closing price
*	Halted

The three auction phase types (PreOpen, PreClose, Auction) behave identically and differ only in the trading-session values reported over the FIX interface; see <<general-mktphs-auction,Auction Phase>>.
The TradeAtLast phase accepts only day limit orders priced at the closing price and crosses them only against one another; see <<general-mktphs-tal,Trade At Last Phase>>.
...
| endTimeRange	| Integer	| Time range, in minutes, to choose a random actual end time from the specified endTime for the phase (this setting is only used by the auction phase types: PreOpen, PreClose, Auction).
...
The range is never considered before the phase's startTime; see <<general-mktphs-auction,Auction Phase>>.
...
{
    "result" : "Successfully inserted a new Venue"
}
----
...
[,http]
----
PUT /api/venues/NewExchange HTTP/1.1
Accept: application/json;charset=UTF-8
Host: localhost
Content-Length: XXX
...
{
    "result" : "Successfully inserted a new Listing"
}
----
...
[[adminsets-data]]
=== Datasources
Manage settings for specific datasources on each market simulator instance.

See <<general-datasource,this section>> for more information about playback of data from datasources.
...
| id	| Numeric	| Unique primary key
| enabled	| Boolean	| Whether or not this datasource is enabled or disabled
| name	| Text	| Name for this datasource, unique to the venue
| venueId	| Text	| Unique venue key for a datasource
| connection	| Text	a| Connection string for the datasource:

* CSV Format - a local server file path
* PSQL Format - a database connection string
* FIX Format - the initiator session ID to use to subscribe to market data, as `+{BeginString}:{SenderCompID}->{TargetCompID}:{SessionQualifier}+` (`+:{SessionQualifier}+` only if configured)

| format	| Text	a| Indicates the type of historic data storage. Supported values:

*	CSV - comma-separated value file
*	PSQL - PostgreSQL (or TimescaleDB) database
*	FIX - market data feed received over a FIX connection

| type	| Text	a| Indicates the datasource format. Supported values:

*	OrderBook - L1/L2 market data format

| repeat	| Boolean	| Whether to start reading from the beginning of the datasource when the end is reached (supported only for files and databases)
| textDelimiter	| Char	| Delimiter used to separate values ("," by default if not specified) (supported only for CSV format)
...
| maxDepthLevels	| Integer	a| Maximum depth levels to read from the datasource. 0 or unspecified indicates to read all levels. For FIX Format, this value is used in tag MarketDepth (264) of each market data subscription.
| listings	| Array	| For FIX Format, list of Symbols to subscribe to market data. An empty list results in no subscriptions. See <<adminsets-data-listings,Listings Sub-List>>.
| randomPriceOnly	| Boolean	a| Whether this datasource is used only as a price source for random order generation (supported only for FIX format)

*   When `false` (the default), orders are derived from the datasource and it can also be used as a random order price source.
*   When `true`, no orders are derived from it, and it is available only as a random order price source
...
| dataSourceId	| Integer	| An identifier of the datasource record which has a column mapping config.
...
| columnTo	| Text	| External datasource column name for mapping data.
...
[[adminsets-data-listings]]
==== Listings Sub-List

Listings that a FIX format datasource subscribes to as an external market data feed. The list is ignored for the CSV and PSQL formats.

[cols="15,15,70"]
|===
| Path | Type |Description

| datasourceId	| Integer	| An identifier of the datasource record which has a listing entry.
Users may omit to send this value in requests.
| symbol	| Text	| The symbol requested from the market data feed (tag 55).

|===

[[adminsets-data-get-sgl]]
==== Get Single Datasource
Request a single market simulator datasource entity.
...
_Example_: Request single datasource
...
{
    "result" : "No such datasource"
}
----

[[adminsets-data-get-multi]]
==== Get Multiple Datasources
Request all market simulator datasources entities.
...

{
    "dataSources" : [
        ...
        },
        {
            ...
            "name" : "XETRA-FEED",
            "venueId" : "XETRA",
            "connection" : "FIXT.1.1:XETRA_FEED_CLIENT->FEED_SERVER",
            "format" : "FIX",
            "type" : "OrderBook",
            "maxDepthLevels" : 1,
            "randomPriceOnly" : false,
            "columnMapping" : [],
            "listings" : [
                {
                    "datasourceId" : 3,
...
[[adminsets-data-add]]
==== Add Datasources
Add a new market simulator datasource entity.
...
_Example_: Request to add a new datasource entity
...
_Example_: Request to add a datasource fed by a FIX market data feed
...
{
    "result" : "Successfully inserted a new datasource"
}
----
...
==== Update Datasources
Update existing market simulator datasource entities.
...
_Example_: Request to update a datasource entity
...
{
    "result" : "Requested update of the datasource with identifier - 1"
}
----
...
_Example_: Negative reply, the update would leave a FIX datasource without a valid session identifier
...
{
    "result" : "connection must be a FIX session identifier in the form BeginString:SenderCompID->TargetCompID[:SessionQualifier] if the format is FIX."
}
----
...
| securityIdSource	| Text	a| Type of security ID entered, can be null (i.e. for FX where we don't require a security ID):

*	CUSIP - US/Canada security identifier
*	SEDOL - UK/Irish security identifier
*	ISINNumber - International Securities Identification Number
*	RICCode - Reuters Instrument Code
*	ExchangeSymbol - Exchange-specific ticker symbol
*	BloombergSymbol - Bloomberg’s security identifier
...
{
    "result" : "Successfully inserted new price seed"
}
----
...
[,http]
----
POST /api/syncpriceseeds HTTP/1.1
Accept: application/json;charset=UTF-8
Host: localhost
----
